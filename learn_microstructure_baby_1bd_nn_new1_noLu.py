...
class build_stiffness_matrix(nn.Module):
    def __init__(self, matrix_dict, X):
        ...
        self.fc1=nn.Linear(2,150,bias=True)
        self.fc2=nn.Linear(150,300,bias=True)
        self.fc3=nn.Linear(300,1,bias=True)
...
save_base_dir="80test_noLu/test/"
model_filename=save_base_dir+"nnParameters.ckpt"
n_samples=50
batch_size=5
learning_rate=1e-5 #usually 1e-3, 2e-3, 5e-3
epochs=2000
step_size=100
gamma=0.5
restart_ep=10000
...
model_filename_restart = './80test_noLu/80test_fc1-2-150_fc2-150-300_fc3-300-1_lr1e-3_gamma050_cont2_lr5e-5/nnParameters.ckpt'
...
for ep in range(epochs):
    ...
    for inputu, inputf in train_loader:
        ...
        loss = torch.sum(torch.pow(E-E_gd,2))
        #E_forsave=E.cpu().detach().numpy()
        #E_gd_forsave=E_gd.cpu().detach().numpy()
        #np.savetxt("./80test_noLu/E_forsave.txt", E_forsave)
        #np.savetxt("./80test_noLu/E_gd_forsave.txt", E_gd_forsave)
        time_end4 = time.time()
    ...
    E_error = torch.sum(torch.pow(E_gd-E,2))
